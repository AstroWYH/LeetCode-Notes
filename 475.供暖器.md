...
4）对于这种2个数组的问题，“分发饼干也类似”，通常只遍历一个数组（像快指针），另一个在内部有条件遍历（像慢指针）；此外，对于这2个数组的两头都可以更长的4种情况，通常可能只需要考虑某个数组的左右两个边界， 算法会自动cover另一个数组的两个边界情况。如此题，只需要考虑houses的左右两个边界即可。
